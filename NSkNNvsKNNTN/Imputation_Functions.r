...
##################################################################################
#### NS-KNN NRMSE Error Function
##################################################################################

# Edited by Justin Lee (2018) to keep NRMSE calculation consistent with NSkNN

Nrmse <- function(imp, mis, true) {
  imp <- as.matrix(imp)
  mis <- as.matrix(mis)
  true <- as.matrix(true)
  MeanOfMet = rowMeans(true)
  missIndex <- which(is.na(mis))
  totNumMV = length(missIndex)
  errvec <- (imp - true)/MeanOfMet
  nrmse <- sqrt(sum(sum(errvec^2))/totNumMV)
