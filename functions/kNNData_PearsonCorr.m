function [imputedData imputedDataWeighted] = kNNData_PearsonCorr(dataMV,K)
...
for targetCol = 1:numCol % i is the target sample
    for neighborCol = 1:numCol % Calculate the Euclidean distances between the target sample (i) and the other samples (j)
        ...
        rho = corr(MVRowsRemoved(:,targetCol),MVRowsRemoved(:,neighborCol));
        distance = (1-abs(rho))/numMetInCalc;
        distIdx(counter,:) = [targetCol distance neighborCol rho];
        counter = counter+1;
    end
end
...
for targetCol = 1:numCol
    ...
    while sum(NNCleared) < numMV
        ...
        for MVidx = 1:numMV % For each missing value in the target sample...
            ...
            rhoNN = distIdxSorted(firstNNIdx:firstNNIdx+Ktemp-1,4); % Rho of nearest neighbors
            while NNCleared(MVidx)==0
                ...
                if isempty(idxNaNinCol)~=1 % If NaN values found...
                    ...
                    rhoNN = [rhoNN; distIdxSorted(firstNNIdx+Ktemp:firstNNIdx+Ktemp+NumNNtoReplace-1,4)];
            ...
            WeightMultiplier = sign(rhoNN').*(1./DistanceNN')/sum(1./DistanceNN);
