function [dataImputed dataImputedWeighted] = NSkNNData_Zero(scaledMV,K,avgMV,stddevMV)
...
% NSkNNData_Zero does not skip neighbors that have NaN values in the same 
...
% Implement NSkNN with zero replacement
...
for targetCol = 1:numCol
    ...
    for MVidx = 1:numMV % For each missing value in the target sample...
        ...
        if isempty(idxNaNinCol)~=1 % If NaN values found...
            ...
            % be imputed, replace with zero

            tempDataMV(rowMV{targetCol}(MVidx),NN(idxNaNinCol)) = (0 - avgMV(rowMV{targetCol}(MVidx)))/stddevMV(rowMV{targetCol}(MVidx));           
        end
